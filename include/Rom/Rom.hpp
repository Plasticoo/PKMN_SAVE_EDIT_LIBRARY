...
{
    if (s == "Rom32kb") { return Type::Rom32kb; }
    if (s == "Rom64kb") { return Type::Rom64kb; }
    // ...
    return Type::Unknown;
}
...
std::unique_ptr<base_of_t<C>> make_templated(const std::string& typeStr)
{
    ...
    const std::map<Type, std::function<std::unique_ptr<base_of_t<C>>()>> factory{
        {Type::Rom32kb, [] { return std::make_unique<C<Rom32kb>>(); } },
        {Type::Rom64kb, [] { return std::make_unique<C<Rom64kb>>(); } },
        // ...
...
struct base_attribute_vector {
    // virtual ~base_attribute_vector() = default;
	...
	virtual auto load_file_rom(std::filesystem::path const& file) -> void = 0;
    virtual auto load_file() -> void = 0;
    ...
    virtual auto get_time_played(struct Gen1::Structs::pkmn_time* dest) const -> void = 0;
    ...
    virtual auto get_badge(enum Gen1::Enums::badges const badge) const -> bool = 0;
    virtual auto set_badge(enum Gen1::Enums::badges const badge) -> void = 0;
    virtual auto get_option(enum Gen1::Enums::options const flag) const -> std::uint8_t = 0;
    virtual auto set_option(enum Gen1::Enums::options const flag) -> void = 0;
    ...
    virtual auto get_item_bag(std::uint8_t const index) const -> struct Gen1::Structs::item* = 0;
    virtual auto set_item_bag(struct Gen1::Structs::item* items, std::uint8_t const index, std::uint8_t const item, std::uint8_t const count) -> void = 0;
    virtual auto get_item_pc_count() const -> std::uint8_t = 0;
    virtual auto get_item_pc(std::uint8_t const index) const -> struct Gen1::Structs::item* = 0;
    virtual auto set_item_pc(struct Gen1::Structs::item* items, std::uint8_t const index, std::uint8_t const item, std::uint8_t const count) -> void = 0;
    virtual auto get_pokemon_party() const -> struct Gen1::Structs::pkmn_party* = 0;
    virtual auto get_pokemon_in_party(std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_party* = 0;
    ...
    virtual auto get_current_pc_box_list() const -> struct Gen1::Structs::pkmn_box* = 0;
    virtual auto get_pokemon_in_current_box(std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_box* = 0;
    virtual auto get_pokemon_in_current_box_name(std::uint8_t index) const -> std::string = 0;
    virtual auto get_pokemon_box(std::uint8_t box) const -> struct Gen1::Structs::pkmn_box* = 0;
    virtual auto get_pokemon_in_box(std::uint8_t box, std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_box* = 0;
...
// template <typename T>
// struct raw_attribute_vector : base_attribute_vector
// {
//     void print() const override { std::cout << typeid(T).name() << std::endl; }
// };
...
struct raw_attribute_vector : base_attribute_vector
{
    ...
    auto load_file_rom(std::filesystem::path const& file) -> void override
    {
        std::ifstream ifs(file, std::ios::binary | std::ios::in);
        if (!ifs) {
            return;
        ...
        if (ifs.read((char*)this->m_rom->data.data(), size)) {
            return;
        }
    }

    auto load_file() -> void override
    {
        ...
        this->current_box_list = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[C::GEN1::OFFSETS::CURRENT_BOX_LIST];
        this->checksum = &this->m_rom->data[C::GEN1::OFFSETS::CHECKSUM];

        this->pocket_item_list = (struct Gen1::Structs::items_bag*)&this->m_rom->data[C::GEN1::OFFSETS::POCKET_ITEM_LIST];
        this->pc_item_list = (struct Gen1::Structs::items_pc*)&this->m_rom->data[C::GEN1::OFFSETS::PC_ITEM_LIST];
        this->time_played = (struct Gen1::Structs::pkmn_time*)&this->m_rom->data[C::GEN1::OFFSETS::TIME_PLAYED];
        this->team_pokemon_list = (struct Gen1::Structs::pkmn_party*)&this->m_rom->data[C::GEN1::OFFSETS::TEAM_POKEMON_LIST];

        this->pc_box[0] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000];
        this->pc_box[1] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000 + (1122 * 1)];
        this->pc_box[2] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000 + (1122 * 2)];
        this->pc_box[3] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000 + (1122 * 3)];
        this->pc_box[4] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000 + (1122 * 4)];
        this->pc_box[5] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x4000 + (1122 * 5)];

        this->pc_box[6] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000];
        this->pc_box[7] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000 + (1122 * 1)];
        this->pc_box[8] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000 + (1122 * 2)];
        this->pc_box[9] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000 + (1122 * 3)];
        this->pc_box[10] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000 + (1122 * 4)];
        this->pc_box[11] = (struct Gen1::Structs::pkmn_box*)&this->m_rom->data[0x6000 + (1122 * 5)];
    }
    ...
    auto get_time_played(struct Gen1::Structs::pkmn_time* dest) const -> void override
    {
    ...
    auto get_badge(enum Gen1::Enums::badges const badge) const -> bool override
    {
        if (this->badges) {
    ...
    auto set_badge(enum Gen1::Enums::badges const badge) -> void override
    {
        if (this->badges) {
    ...
    auto get_option(enum Gen1::Enums::options const flag) const -> std::uint8_t override
    {
        if (!this->options) {
            return 0;
        }
    ...
    auto set_option(enum Gen1::Enums::options const flag) -> void override
    {
        if (!this->options) {
            return;
        }
    ...
    auto get_item_bag(std::uint8_t const index) const -> struct Gen1::Structs::item* override
    {
    ...
    auto set_item_bag(struct Gen1::Structs::item* items, std::uint8_t const index, std::uint8_t const item, std::uint8_t const count) -> void override
    {
        if (index <= C::GEN1::SIZES::BAG_ITEM) {
    ...
    auto get_item_pc(std::uint8_t const index) const -> struct Gen1::Structs::item* override
    {
    ...
    // TODO: This is not doing what it is supposed to do
    auto set_item_pc(struct Gen1::Structs::item* items, std::uint8_t const index, std::uint8_t const item, std::uint8_t const count) -> void override
    {
        if (index <= C::GEN1::SIZES::PC_ITEM) {
            items[index].index = item;
            items[index].count = count;
    ...
    auto get_pokemon_party() const -> struct Gen1::Structs::pkmn_party* override
    {
    ...
    auto get_pokemon_in_party(std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_party* override
    {
        if (this->team_pokemon_list) {
    ...
    auto get_current_pc_box_list() const -> struct Gen1::Structs::pkmn_box* override
    {
    ...
    auto get_pokemon_in_current_box(std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_box* override
    {
        if (this->current_box_list) {
    ...
    auto get_pokemon_box(std::uint8_t box) const -> struct Gen1::Structs::pkmn_box* override
    {
        if (this->pc_box[box]) {
    ...
    auto get_pokemon_in_box(std::uint8_t box, std::uint8_t index) const -> struct Gen1::Structs::pkmn_data_box* override
    {
  ...
  private:
    ...
    struct Gen1::Structs::items_bag* pocket_item_list;
    struct Gen1::Structs::items_pc* pc_item_list;
    struct Gen1::Structs::pkmn_time* time_played;
    struct Gen1::Structs::pkmn_party* team_pokemon_list;
    struct Gen1::Structs::pkmn_box* current_box_list;
    struct Gen1::Structs::pkmn_box* pc_box[12];
};
